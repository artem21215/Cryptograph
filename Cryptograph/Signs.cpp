...
namespace Sign_NS{
    ...
    template<typename Type>
    void GOST<Type>::signing(const string& inputFileName,
                                Users_NS::User<Type>& user) {
        std::ifstream input( inputFileName, std::ifstream::binary );
        char message;
        string fullFile;
        ...
        constexpr int lenElGamalG = 90;
        auto k = findRandSafePrime<LongArithmetic>(lenElGamalG) % (user.bigQ.getString().size()-2) + 1;
        auto r = fastPow(user.aGost,k,user.bigModP);
        //auto u = (fileHash - x * r) % (user.getMod()-1);
        auto calculatorMessage = [r,k](LongArithmetic message, LongArithmetic key, LongArithmetic mod){return (k*message + key * r) % mod;};
        auto s = user.calcMessage(fileHash, calculatorMessage, "EC");
    ...
    template<typename Type>
    bool GOST<Type>::checkSign(const string& inputFileName,
                                  Users_NS::User<Type>& targetUser) {
        LongArithmetic fileSize;
        std::ifstream input( inputFileName, std::ifstream::binary );
        char message;
    ...
    template class GOST<LongArithmetic>;
}